...
- `Release gate` is **designed** to be the only authority that turns CI evidence into a merge or deploy decision, and must never be weakened -- but it is not yet operating (it has never published a check, and deploys bypass it entirely via Vercel's Git integration; see `docs/ci/current-state.md`). Do not treat it as a safety net when judging the risk of a change: anything merged to `main` is live in production. As designed it runs from protected `main`, refuses on the first mismatch (repository id, App id, installation id, workflow id, SHA tuple, policy version, image digest, missing or stale evidence), and is never overridden by editing policy or disabling a check.
