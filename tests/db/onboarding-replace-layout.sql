...
DECLARE
  ...
  v_restaurant_id constant uuid := '00000000-0000-4000-8000-00000000a002';
  -- Restaurant A has bookings in the fixtures.
  v_booked_restaurant_id constant uuid := '00000000-0000-4000-8000-00000000a001';
...
BEGIN
  SELECT jsonb_build_object(
    'zones', (SELECT count(*) FROM public.zones WHERE restaurant_id = v_booked_restaurant_id),
    'tables', (SELECT count(*) FROM public.table_inventory WHERE restaurant_id = v_booked_restaurant_id)
  ) INTO v_other_before;

  -- 1. First replace creates zones, tables and allowed capacities.
  v_first := public.onboarding_replace_layout(v_restaurant_id, v_zones, v_tables);
  ...
  SELECT id INTO v_main_zone FROM public.zones WHERE restaurant_id = v_restaurant_id AND name = 'Main Dining';
  IF (SELECT zone_id FROM public.table_inventory WHERE restaurant_id = v_restaurant_id AND table_number = 'T3')
  ...
  IF (SELECT lower(z.name) FROM public.table_inventory t JOIN public.zones z ON z.id = t.zone_id
      WHERE t.restaurant_id = v_restaurant_id AND t.table_number = 'T2') <> 'terrace' THEN
    RAISE EXCEPTION 'zone_name match is not case-insensitive' USING ERRCODE = 'NB001';
  END IF;
  SELECT count(*) INTO v_count FROM public.allowed_capacities WHERE restaurant_id = v_restaurant_id;
  IF v_count <> 3 THEN
  ...
  FROM public.table_inventory WHERE restaurant_id = v_restaurant_id AND table_number = 'T1';

  -- 2. Replaying the same payload is a no-op: same ids, no duplicate rows, no row rewrite.
  v_second := public.onboarding_replace_layout(v_restaurant_id, v_zones, v_tables);
  IF v_second <> v_first THEN
    RAISE EXCEPTION 'replay changed the result: % vs %', v_second, v_first USING ERRCODE = 'NB001';
  END IF;
  SELECT count(*) INTO v_count FROM public.zones WHERE restaurant_id = v_restaurant_id;
  IF v_count <> 2 THEN
    RAISE EXCEPTION 'replay duplicated zones (%)', v_count USING ERRCODE = 'NB001';
  END IF;
  SELECT count(*) INTO v_count FROM public.table_inventory WHERE restaurant_id = v_restaurant_id;
  IF v_count <> 3 THEN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM public.zones WHERE restaurant_id = v_restaurant_id AND name = 'Terrace') THEN
  ...
  v_raised := false;
  BEGIN
    PERFORM public.onboarding_replace_layout(v_restaurant_id, '[{"name":"A"},{"name":"a"}]', '[]');
  ...
  BEGIN
    PERFORM public.onboarding_replace_layout(
      v_restaurant_id, '[{"name":"Main Dining"}]',
  ...
  BEGIN
    PERFORM public.onboarding_replace_layout(
      v_restaurant_id, '[{"name":"Main Dining"}]', '[{"table_number":"T1","capacity":"two"}]');
  EXCEPTION WHEN SQLSTATE '22023' THEN
    v_raised := true;
  END;
  IF NOT v_raised THEN
  ...
  v_raised := false;
  BEGIN
    PERFORM public.onboarding_replace_layout(v_restaurant_id, '[]', '[]');
  EXCEPTION WHEN SQLSTATE '22023' THEN
    v_raised := true;
  END;
  IF NOT v_raised THEN
  ...
  v_raised := false;
  BEGIN
    PERFORM public.onboarding_replace_layout(v_booked_restaurant_id, '[{"name":"X"}]', '[]');
  ...
  v_raised := false;
  BEGIN
    PERFORM public.onboarding_replace_layout(gen_random_uuid(), '[{"name":"X"}]', '[]');
