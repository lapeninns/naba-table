...
describe('runBookingCreatePrecommitContext', () => {
  ...
  it('rejects a Sunday Roast selection when the venue setting is off', async () => {
    const deps = buildContinueDeps({
      scheduleGateRunner: vi.fn(async () => ({
        kind: 'continue',
        startTime: '12:30',
  ...
  it('allows a Sunday Roast selection for an enabled venue on Sunday', async () => {
    const deps = buildContinueDeps({
      scheduleGateRunner: vi.fn(async () => ({
        kind: 'continue',
        startTime: '12:30',
    ...
    ).resolves.toMatchObject({ kind: 'continue', startTime: '12:30' });
  });
