...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'table_inventory_restaurant_table_number_key'
      AND conrelid = 'public.table_inventory'::regclass
  ) THEN
    ALTER TABLE public.table_inventory
      ADD CONSTRAINT table_inventory_restaurant_table_number_key
      UNIQUE (restaurant_id, lower(table_number));
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'zones_restaurant_name_key'
      AND conrelid = 'public.zones'::regclass
  ) THEN
    ALTER TABLE public.zones
      ADD CONSTRAINT zones_restaurant_name_key
