...
const getSortLabel = (sortKey: BookingSortKey) => {
  if (sortKey === 'time') return 'Time';
  if (sortKey === 'party') return 'Party size';
  return 'Guest name';
};

const getSortDirLabel = (sortDir: BookingSortDir) => (sortDir === 'asc' ? 'Ascending' : 'Descending');

function usePrintBodyClass(className: string) {
  useEffect(() => {
...
type AutoPrintOptions = {
  summary: OpsTodayBookingsSummary | null;
  isSummaryReady: boolean;
  isFetching: boolean;
};

function useAutoPrint({ summary, isSummaryReady, isFetching }: AutoPrintOptions) {
  const [hasPrinted, setHasPrinted] = useState(false);

  useEffect(() => {
    if (!summary || !isSummaryReady || hasPrinted || isFetching) return;
    const handle = window.setTimeout(() => {
      window.print();
      setHasPrinted(true);
    }, 250);
    return () => window.clearTimeout(handle);
  }, [hasPrinted, isFetching, isSummaryReady, summary]);
}
...
function usePrintDocumentTitle({ summary, isSummaryReady }: PrintTitleOptions) {
  useEffect(() => {
...
function useAllowTableAssignments(summary: OpsTodayBookingsSummary | null, isSummaryReady: boolean) {
  return useMemo(() => {
...
function useCurrentOpsNow(summary: OpsTodayBookingsSummary | null) {
  return useMemo(
...
}: FilteredBookingsOptions) {
  return useMemo(() => {
    if (!summary || !isSummaryReady) return [];
    return filterBookings({ summary, filter, searchQuery, now, allowTableAssignments });
  }, [allowTableAssignments, filter, isSummaryReady, now, searchQuery, summary]);
}

type SortedBookingsOptions = {
  filter: BookingFilter;
  filteredBookings: OpsTodayBooking[];
  sortKey: BookingSortKey;
  sortDir: BookingSortDir;
};

function useSortedBookings({ filter, filteredBookings, sortKey, sortDir }: SortedBookingsOptions) {
  return useMemo(() => {
    if (filter === 'all') {
...
function filterBookings({ summary, filter, searchQuery, now, allowTableAssignments }: FilterBookingsOptions) {
...
function PrintHeader({ restaurantName, readableDate, onPrint }: PrintHeaderProps) {
  return (
    <header className={styles.header}>
      ...
      <div className={`${styles.printControls} ${styles.headerActions}`}>
        <Button variant="outline" size="sm" onClick={onPrint} aria-label="Print bookings">
          Print
        </Button>
      </div>
    </header>
...
function PrintMetaRow({ filterLabel, sortLabel, sortDirLabel, searchQuery }: PrintMetaRowProps) {
  return (
...
function PrintTable({ bookings, timezone }: PrintTableProps) {
      ...
      <table className={styles.table}>
        ...
        <tbody>
          {bookings.map((booking) => {
            ...
            const timeLabel = formatTimeRange(booking.startTime, booking.endTime, timezone);
...
function NoRestaurantAccessState() {
  return (
...
function LoadingState({ targetDate }: LoadingStateProps) {
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-6 text-center">
      <div className="max-w-md space-y-2">
...
function ErrorState({ targetDate }: ErrorStateProps) {
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-6 text-center">
      <div className="max-w-md space-y-2">
...
type ViewStateOptions = {
  ...
  summaryQuery: ReturnType<typeof useOpsTodaySummary>;
  summary: OpsTodayBookingsSummary | null;
  isSummaryMismatch: boolean;
  targetDate: string | null;
};

function getPrintViewState({
  restaurantId,
  summaryQuery,
  summary,
  isSummaryMismatch,
...
export function OpsBookingsPrintView({ params }: OpsBookingsPrintViewProps) {
  ...
  usePrintBodyClass(styles.printBody);
  useAutoPrint({ summary, isSummaryReady, isFetching: summaryQuery.isFetching });
  usePrintDocumentTitle({ summary, isSummaryReady });

  const allowTableAssignments = useAllowTableAssignments(summary, isSummaryReady);
  const now = useCurrentOpsNow(summary);
  ...
  const sortedBookings = useSortedBookings({ filter, filteredBookings, sortKey, sortDir });

  const viewState = getPrintViewState({
    restaurantId,
    summaryQuery,
    summary,
    isSummaryMismatch,
  ...
  const sortDirLabel = getSortDirLabel(sortDir);
  ...
  return (
    <div id="ops-print-root" className={styles.printRoot}>
      <div className={styles.page}>
        ...
        />
        <PrintTable bookings={sortedBookings} timezone={summary.timezone} />
      </div>
    </div>
  );
}
