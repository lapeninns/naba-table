...
function BookingCard({
  ...
  // Determine card styles based on urgency - using DesignSystem.md tokens
  const cardStyle = useMemo(() => {
    const base = {
      borderRadius: 'var(--radius-lg)',
      transition: 'box-shadow 0.2s ease, transform 0.2s ease, border-color 0.2s ease',
    };
    if (isDone) return { ...base, borderColor: 'var(--slate-100)', backgroundColor: 'var(--slate-50)' };
    if (timeUrgency?.type === 'late') return { ...base, borderColor: 'var(--red-500)', backgroundColor: 'rgba(239, 68, 68, 0.05)' };
    if (timeUrgency?.type === 'overdue') return { ...base, borderColor: 'var(--amber-500)', backgroundColor: 'rgba(245, 158, 11, 0.05)' };
    if (timeUrgency?.type === 'soon') return { ...base, borderColor: 'var(--amber-500)', backgroundColor: 'rgba(245, 158, 11, 0.03)' };
    return { ...base, borderColor: 'var(--border)', backgroundColor: 'var(--card)' };
  }, [isDone, timeUrgency]);

  return (
    <div
      className={cn(
        "group relative flex flex-col gap-3 border p-3 sm:gap-4 sm:p-4 sm:flex-row sm:items-center",
        isLoading && "opacity-60 pointer-events-none animate-pulse",
        !isLoading && "hover:shadow-[var(--shadow-lg)] hover:-translate-y-0.5 active:translate-y-0"
      )}
      style={cardStyle}
    >
      ...
      {isLoading && (
        <div
          ...
          style={{ borderRadius: 'var(--radius-lg)', backgroundColor: 'rgba(241, 245, 249, 0.5)' }}
        >
          <div
      ...
      <div className="flex min-w-[100px] shrink-0 flex-row items-center gap-4 sm:flex-col sm:items-start sm:gap-1">
        <div className="flex flex-col">
          <span
            className="font-mono text-lg font-bold leading-none tracking-tight"
            style={{ color: isDone ? 'var(--slate-400)' : 'var(--foreground)' }}
          >
            {startTimeStr}
          </span>
          <span
        ...
        <div className="hidden h-px w-8 sm:block" style={{ backgroundColor: 'var(--slate-100)' }} />

        <div className="flex items-center gap-1.5">
          <Users className="h-3.5 w-3.5" style={{ color: isDone ? 'var(--slate-300)' : 'var(--slate-400)' }} />
          <span
            className="text-sm font-medium"
            style={{ color: isDone ? 'var(--slate-400)' : 'var(--slate-700)' }}
          >
            {booking.partySize} guests
          </span>
        </div>

        {/* Time Urgency Badge - using DesignSystem.md tokens */}
        {timeUrgency && (
          <div
            className="flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold animate-pulse"
            style={{
              borderRadius: 'var(--radius-full)',
              backgroundColor: timeUrgency.type === 'late' ? 'rgba(239, 68, 68, 0.1)' :
                timeUrgency.type === 'overdue' ? 'rgba(245, 158, 11, 0.1)' :
                  timeUrgency.type === 'soon' ? 'rgba(245, 158, 11, 0.05)' :
                    'var(--brand-blue-subtle)',
              color: timeUrgency.type === 'late' ? 'var(--red-500)' :
                timeUrgency.type === 'overdue' ? 'var(--amber-500)' :
                  timeUrgency.type === 'soon' ? 'var(--amber-500)' :
                    'var(--brand-blue)'
            }}
          >
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            style={{
              borderRadius: 'var(--radius-full)',
              backgroundColor: isDone ? 'var(--slate-100)' : 'var(--brand-blue-subtle)',
              color: isDone ? 'var(--slate-400)' : 'var(--brand-blue)'
            }}
          >
            {guestInitials}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span
                className={cn("text-base font-semibold", isDone && "line-through")}
                style={{
                  color: isDone ? 'var(--slate-500)' : 'var(--foreground)',
                  textDecorationColor: isDone ? 'var(--slate-300)' : undefined
                }}
              >
              ...
              {booking.loyaltyTier && (
                <Sparkles
                  ...
                  style={{
                    color: booking.loyaltyTier === 'platinum' ? 'var(--brand-blue)' :
                      booking.loyaltyTier === 'gold' ? 'var(--amber-500)' : 'var(--slate-400)'
                  }}
                  fill="currentColor"
                />
              )}
      ...
      {/* ZONE 3: ACTIONS */}
      <div
        className="flex shrink-0 items-center justify-between gap-2 pt-3 sm:mt-0 sm:flex-col sm:items-end sm:border-0 sm:pt-0 sm:gap-3"
        style={{ borderTopWidth: '1px', borderTopColor: 'var(--slate-100)' }}
      >
        ...
        {!isDone ? (
          <>
            {(!isSeated) ? (
              <button
                className="flex-1 h-11 inline-flex items-center justify-center font-semibold text-sm transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed sm:flex-none sm:h-auto sm:px-4 sm:py-2 group/btn touch-manipulation"
                style={{
                  borderRadius: 'var(--radius-full)',
              ...
              >
                ...
                <span
                  className="ml-2 hidden px-1 py-0.5 text-[9px] font-mono opacity-60 group-hover/btn:inline"
                  style={{ borderRadius: 'var(--radius-sm)', backgroundColor: 'var(--brand-blue-hover)' }}
                >S</span>
              </button>
            ) : (
              <button
              ...
              >
                ...
                <span
                  className="ml-2 hidden px-1 py-0.5 text-[9px] font-mono opacity-60 group-hover/btn:inline"
                  style={{ borderRadius: 'var(--radius-sm)', backgroundColor: 'rgba(5, 150, 105, 0.1)' }}
                >F</span>
              </button>
            )}
            ...
            <div className="flex gap-1">
              ...
              {!isSeated && (
                <button
                  className="h-11 w-11 inline-flex items-center justify-center transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed sm:h-8 sm:w-8 touch-manipulation"
                  style={{
                    borderRadius: 'var(--radius-full)',
                    color: 'var(--slate-400)',
          ...
          <button
            className="w-full inline-flex items-center justify-center font-semibold text-sm cursor-default sm:w-auto px-4 py-2"
            style={{
              borderRadius: 'var(--radius-full)',
