  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      <div className="guest-boundary flex w-full max-w-4xl items-center justify-between gap-4 py-4 sm:py-5">
        ...
        <nav aria-label="Auth navigation" className="hidden flex-1 items-center justify-center md:flex">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <Link href="/support" className={ctaClasses("ghost")}>Need help?</Link>
          <Link href="/restaurants" className={ctaClasses()}>Explore tables</Link>
        </div>
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-50 bg-foreground/60 backdrop-blur-sm">
          <div className="absolute inset-x-0 bottom-0 flex flex-col">
            <div className="rounded-t-3xl bg-background px-6 py-6 shadow-2xl" role="dialog" aria-modal="true" id={MOBILE_MENU_ID}>
              <div className="flex items-center justify-between pb-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-foreground/60">Navigation</p>
              ...
              <div className="mt-6 space-y-3">
                <Link href="/restaurants" onClick={closeMenu} className={cn(ctaClasses(), "w-full justify-center")}>Explore tables</Link>
                <Link href="/support" onClick={closeMenu} className={cn(ctaClasses("ghost"), "w-full justify-center")}>Need help?</Link>
              </div>
            </div>
          </div>
        </div>
