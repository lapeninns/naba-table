...
import type { ManualAssignmentTable, ManualAssignmentConflict, ManualAssignmentContextHold } from "@/services/app/bookings";
...
export function useAssignmentContext({ bookingId, enabled = true }: UseAssignmentContextOptions) {
    ...
    const query = useQuery<AssignmentContext, Error>({
        ...
        queryFn: async () => {
            ...
            const response = await fetch(`/api/app/bookings/${bookingId}/assignment-context`);
            if (!response.ok) {
