...
const OPS_BOOKINGS_BASE = '/api/app/bookings';
...
async function fetchContextVersion(bookingId: string): Promise<string | null> {
  try {
    const res = await fetchJson<ManualAssignmentContext>(`/api/app/bookings/${bookingId}/manual-context`);
  ...
  return {
    ...
    async getManualAssignmentContext(bookingId, _options) {
      const context = await fetchJson<ManualAssignmentContext>(
        `/api/app/bookings/${bookingId}/manual-context`,
        {
          method: 'GET',
        },
      );
    ...
    async assignTablesDirect({ bookingId, tableIds, idempotencyKey, requireAdjacency }) {
      ...
      }>(`/api/app/bookings/${bookingId}/assign-tables`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    async unassignTablesDirect({ bookingId, tableIds }) {
      return fetchJson<{ success: true; removedCount: number }>(`/api/app/bookings/${bookingId}/assign-tables`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
