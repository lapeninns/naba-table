...
export default async function RestaurantSettingsLayout({ children }: { children: ReactNode }) {
  ...
  if (!user) {
    redirect(withRedirectedFrom('/auth/signin', '/settings/restaurant/profile'));
  }

  return (
    <RestaurantSettingsPageShell
      title="Restaurant"
      description="Configure the restaurant profile, operating hours, service periods, booking occasions, tables, and team access."
    >
      {children}
    </RestaurantSettingsPageShell>
  );
