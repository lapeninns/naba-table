...
async function prefetchOpsCustomers(queryClient: QueryClient, params: URLSearchParams) {
  ...
  try {
    ...
    console.error('[ops/customers] prefetch failed', error);
  }
}
...
export default async function OpsCustomersPage({ searchParams }: { searchParams?: Promise<Record<string, string>> }) {
  ...
  if (error) {
    console.error('[ops/customers] failed to resolve auth', error.message);
  }

  if (!user) {
    redirect(withRedirectedFrom('/auth/signin', '/app/customers'));
  }
  ...
  return (
    <HydrationBoundary state={dehydratedState}>
      <div className="mx-auto w-full max-w-[80vw] px-4 py-6 sm:px-6 lg:px-8">
