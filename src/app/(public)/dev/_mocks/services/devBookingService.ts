...
export class DevBookingService implements BookingService {
  ...
  async getRestaurantSmsDeliveryFeed(params: {
    ...
    const attempts: Extract<OpsSmsDeliveryFeedResponse, { ok: true }>['attempts'] = [
      ...
      {
        messageSid: 'SMDEV333',
        recipientPhone: '+447700900333',
        bookingId: null,
        smsType: 'booking_confirmation',
        provider: 'mock',
        currentStatus: 'sent',
        currentOccurredAt: new Date(now.getTime() - 13 * 60 * 60_000).toISOString(),
        isStale: true,
        stuckForMs: 13 * 60 * 60_000,
        booking: null,
        events: [
          {
            id: 'dev-sms-3-queued',
            ...
            status: 'queued',
            provider: 'mock',
            occurredAt: new Date(now.getTime() - 13.5 * 60 * 60_000).toISOString(),
            error: null,
            metadata: {
          ...
          },
          {
            ...
            status: 'sent',
            provider: 'mock',
            occurredAt: new Date(now.getTime() - 13 * 60 * 60_000).toISOString(),
            error: null,
            metadata: {
              source: 'delivery_reconciler_poll',
              polledStatus: 'sent',
    ...
    return {
      ...
      summary: {
        ...
        stuckInFlight: filtered.filter((attempt) => attempt.isStale === true).length,
      },
    };
  }
