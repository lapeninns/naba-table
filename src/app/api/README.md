...
- Post-sign-in destinations are host-aware in proxy/auth routes:
  - Root-origin logins default to `/guest/dashboard`.
  - App-origin logins default to `/app/dashboard`.

## Adding or changing ops APIs

1. Add the service key to `OPS_API_SERVICES` in `src/proxy.ts`.
...
- Static and framework assets (`/_next`, `favicon.ico`, `robots.txt`, `sitemap.xml`) bypass proxy redirects.
