...
describe("POST /api/app/bookings", () => {
  ...
  it("creates a walk-in booking without contact info", async () => {
    ...
    const schemaSpy = vi
      ...
      .mockReturnValue(payload as unknown as z.infer<typeof opsWalkInBookingSchema>);


    const request = new NextRequest("http://localhost/api/app/bookings", {
      method: "POST",
...
describe("GET /api/app/bookings", () => {
  ...
  it("returns empty list when user has no memberships", async () => {
    ...
    const request = new NextRequest("http://localhost/api/app/bookings?page=1&pageSize=5");
    const response = await GET(request);
  ...
  it("lists bookings for restaurants the user belongs to", async () => {
    ...
    const request = new NextRequest(`http://localhost/api/app/bookings?restaurantId=${RESTAURANT_ID}&page=1&pageSize=10`);
    const response = await GET(request);
    expect(response.status).toBe(200);
    const json = await response.json();
  ...
  it("filters results using name or email search", async () => {
    ...
    const request = new NextRequest(
      `http://localhost/api/app/bookings?restaurantId=${RESTAURANT_ID}&page=1&pageSize=5&query=${encodeURIComponent(rawSearch)}`,
    );
