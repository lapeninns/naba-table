...
export function ReceiptClient({ reservationId, hasSession }: ReceiptClientProps) {
  ...
  const statusTone = useMemo(() => {
    ...
    if (['confirmed', 'completed', 'checked_in'].includes(reservation.status)) return { label: 'Confirmed', tone: 'success' as const };
    if (['pending', 'pending_allocation'].includes(reservation.status)) return { label: 'Pending Confirmation', tone: 'warning' as const };
  ...
  if (isError || !reservation) {
    return (
      <BookingDetailShell>
        <InlineAlert tone="danger">We couldn’t load your receipt. Please try the link again.</InlineAlert>
        <div className="pt-4">
  ...
  const statCards = (
    <div className="grid gap-[var(--space-4)] sm:grid-cols-3">
      ...
      <DetailStatCard icon={Clock} label="Time" value={formatTime(reservation.startAt)} subtext={venue.timezone} />
      <DetailStatCard icon={Users} label="Party" value={`${reservation.partySize}`} subtext={reservation.partySize === 1 ? 'Guest' : 'Guests'} />
    </div>
  );
  ...
  const guestInfo = (
    <InfoPanel
      ...
      rows={[
        ...
        ...(reservation.customerEmail ? [{ icon: Mail, label: 'Email', value: reservation.customerEmail }] : []),
      ]}
    />
  );
    ...
    <BookingDetailShell>
      ...
      <InlineAlert tone="info">
        <div className="flex items-center gap-[var(--space-2)]">
          ...
          <p className="text-[length:var(--font-size-sm)]">A confirmation email has been sent to your inbox.</p>
        </div>
      </InlineAlert>
