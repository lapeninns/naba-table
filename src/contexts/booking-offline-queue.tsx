...
export function BookingOfflineQueueProvider({ children }: { children: ReactNode }) {
  ...
  const isQueued: BookingOfflineQueueContextValue['isQueued'] = useCallback(
    (bookingId) => {
  ...
  const getPendingAction: BookingOfflineQueueContextValue['getPendingAction'] = useCallback(
    (bookingId) => {
  ...
  const flushQueue = useCallback(async () => {
    ...
    try {
      while (true) {
        ...
        try {
          await next.perform();
          const latest = queueRef.current.filter((entry) => entry.id !== next.id);
  ...
  return (
    <BookingOfflineQueueContext.Provider value={value}>
      {children}
    </BookingOfflineQueueContext.Provider>
  );
}
