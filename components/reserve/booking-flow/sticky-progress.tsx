...
export const StickyProgress: React.FC<StickyProgressProps> = ({
  ...
  const summaryAriaLabel = [summaryPrimaryText, summarySecondaryParts.join(", ")].filter(Boolean).join(". ");
  ...
  const leftAction = actions.length > 1 ? actions[0] : actions.length === 0 ? defaultBackAction : null;
        ...
        >
          <div className="flex flex-col gap-3 px-4 py-3 sm:px-5">
            ...
            <div className="flex items-center justify-between gap-3 md:gap-5">
              {/* Left circular back (or fallback) */}
                ...
                {leftAction && (() => {
                  ...
                  return (
                    <Button
                      ...
                      data-tooltip-content={leftAction.label}
              ...
              {/* Center (summary on steps 1..n-1, special icons on last step) */}
              <div className="flex min-w-0 flex-1 items-center justify-center px-1 text-center md:justify-start md:px-4 md:text-left">
                {current < safeTotal ? (
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-sm leading-tight text-[color:var(--color-text-secondary)] md:justify-start">
                      {summarySecondaryParts.map((part, index) => (
                        <React.Fragment key={`${part}-${index}`}>
                          {index > 0 && <span aria-hidden className="text-[color:var(--color-text-secondary)]">•</span>}
                          <span>{part}</span>
                        </React.Fragment>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    {(() => {
                      const cal = actions.find((a) => a.icon === "Calendar" || /calendar/i.test(a.id));
                      const wal = actions.find((a) => a.icon === "Wallet" || /wallet/i.test(a.id));
                      const candidates = [cal, wal].filter(Boolean) as typeof actions;
              ...
              {/* Right circular primary (prefer last provided action) */}
              <div className="relative flex min-h-[44px] min-w-[44px] items-center justify-center sm:min-h-[48px] sm:min-w-[48px]">
                {(() => {
                  ...
                  return (
                    <Button
                      ...
                      data-tooltip-id="tooltip"
                      data-tooltip-content={rightAction.label}
            ...
            <div className="flex items-center gap-3 pt-1">
              {Array.from({ length: safeTotal }, (_, i) => i + 1).map((i) => (
                <span
                  key={i}
