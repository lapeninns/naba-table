# Agent setup for `nabatableLP`

This is a deepsec scanning workspace. Project `nabatableLP` was just registered
(target: `..`). Setup is incomplete — `data/nabatableLP/INFO.md`
...
2. **Fill in `data/nabatableLP/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id nabatableLP
pnpm deepsec process --project-id nabatableLP
```
