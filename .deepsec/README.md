...
Currently configured project: `nabatableLP` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/nabatableLP/SETUP.md` to fill in
   `data/nabatableLP/INFO.md`.
...
`--project-id nabatableLP` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/nabatableLP/`.
