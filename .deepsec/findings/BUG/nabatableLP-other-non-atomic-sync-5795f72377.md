...
`syncGoogleBusinessProfileCanonicalBusinessInfo` now writes the canonical Google
Business Profile mirror through the service-role
`replace_gbp_canonical_business_info` RPC instead of deleting and reinserting
provider rows table by table from application code.

Evidence:

- `server/google-business-profile/business-info.ts` calls
  `replace_gbp_canonical_business_info`.
- `supabase/migrations/20260516090500_atomic_gbp_canonical_business_info.sql`
  defines the transactional replacement RPC and service-role execution grant.
- `tests/server/google-business-profile-business-info.test.ts` covers RPC
  payload behavior, optional service-item preservation, and migration presence.
- `pnpm exec vitest run tests/server/google-business-profile-business-info.test.ts`
