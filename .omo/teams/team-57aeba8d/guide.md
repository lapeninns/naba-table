# Team square-duration-services - Member Field Manual
...
- Team: **square-duration-services** (id `1abbf387-44df-4205-9948-e4f72b245f4b`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-57aeba8d/team.json`
...
**You are a member of team square-duration-services, not a generic assistant.** You own exactly one
...
- **A** "service-setup" (area) - Official Square help pages for service creation, duration, variations, pricing and duration fields -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_service_setup` | agent `/root/sq_service_setup` [pending]
- **C** "blocked-time" (area) - Official Square pages for blocking time, personal events, unavailable time, calendar blocks and availability effects -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_blocked_time` | agent `/root/sq_blocked_time` [pending]
- **B** "processing-time" (area) - Official Square pages documenting processing time, bookable segments, concurrent availability and service timing -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_processing_time` | agent `/root/sq_processing_time` [pending]
- **F** "sitemap-dev-ui" (perspective) - Official Square sitemap, product walkthroughs and developer docs exposing duration-related names, enums or UI terminology -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_sitemap_dev_ui` | agent `/root/sq_sitemap_dev_ui` [pending]
- **E** "booking-policies" (area) - Official Square booking policies and online booking timing constraints that interact with duration and availability -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_booking_policies` | agent `/root/sq_booking_policies` [pending]
- **D** "transition-buffers" (area) - Official Square pages for transition time, buffer time before or after appointments, setup and cleanup terminology -> 10+ query log, full-page citations, exact short quotes, UI labels/helper/validation copy, implications, CLAIMS and EXPAND | task `sq_transition_buffers` | agent `/root/sq_transition_buffers` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-57aeba8d/artifacts`
