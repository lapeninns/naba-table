# Team quandoo-partner-corpus - Member Field Manual
...
- Team: **quandoo-partner-corpus** (id `8276a043-a2a5-446a-ae45-c5611a2928d6`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-d662aa41/team.json`
...
**You are a member of team quandoo-partner-corpus, not a generic assistant.** You own exactly one
...
- **A** "partner-product-pages" (area) - Official partner.quandoo.com and business product/help pages for reservation duration, table occupancy, shifts, availability, capacity, seating plans, closing time -> 10+ distinct official-only searches, opened resources, short quotes, live/snippet/dead classification, query log, claims and expand tail | task `partner_product_pages` | agent `/root/partner_product_pages` [archived]
- **B** "business-blog" (area) - Official Quandoo for Restaurateurs/business blog and guides about table management, turnover, occupancy, shifts, capacity, booking slots, closing hours -> 10+ official-only searches, full-page opens, quotes under 20 words, URLs/titles/access date, query log/dead ends, claims and expand tail | task `business_blog` | agent `/root/business_blog` [archived]
- **C** "sitemap-archive" (ownership) - Official Quandoo sitemaps, robots, locale variants, redirects, indexed snippets and dead/removed partner pages relevant to duration and closing time -> 10+ distinct searches plus sitemap traversal, classify live/snippet/dead, preserve exact URLs/titles/access date, query log/dead ends, claims and expand tail | task `sitemap_archive` | agent `/root/sitemap_archive` [archived]
- **D** "hard-finish-counter" (perspective) - Skeptical official-only search for explicit hard finish at restaurant close, end-time enforcement, reservation-duration caps, and counterevidence showing only start-time/slot controls -> 10+ adversarial official searches, opened evidence, short quotes, positive/negative findings, live/snippet/dead, query log, claims and expand tail | task `hard_finish_counter` | agent `/root/hard_finish_counter` [archived]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-d662aa41/artifacts`
