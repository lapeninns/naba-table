# Team thefork-help-periods - Member Field Manual
...
- Team: **thefork-help-periods** (id `59d55134-bc85-4525-ab19-8bbc6df65801`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-d526328f/team.json`
...
**You are a member of team thefork-help-periods, not a generic assistant.** You own exactly one
...
- **A** "ui-terminology" (ownership) - Exact English labels and helper wording for creating or editing bookable meal periods -> 10+ varied official-domain queries, full-page fetches, quotes under 20 words, URLs, CLAIMS and EXPAND tails | task `thefork_ui_terms` | agent `/root/thefork_ui_terms` [pending]
- **B** "site-indexes" (area) - Official robots, sitemaps, help-center category indexes, and page enumeration for services/opening/booking availability -> Index inventory, every relevant page fetched, query log, dead ends, CLAIMS and EXPAND tails | task `thefork_site_indexes` | agent `/root/thefork_site_indexes` [pending]
- **C** "term-countersearch" (perspective) - Alternate English terminology: service, shift, meal period, schedule, opening hours, closing time, reservation slots -> 10+ operator-varied counter-searches, terminology map, exact quotes, pages, CLAIMS and EXPAND tails | task `thefork_term_countersearch` | agent `/root/thefork_term_countersearch` [pending]
- **E** "legacy-redirects" (area) - Legacy and alternate official TheFork/TheFork Manager English help domains, redirects, archived public paths, and login walls -> 10+ domain/path searches, redirect and access notes, full public pages, exact quotes, CLAIMS and EXPAND tails | task `thefork_legacy_redirects` | agent `/root/thefork_legacy_redirects` [pending]
- **D** "closure-availability" (area) - Closing times, exceptional closures, booking availability, and meal-period boundary behavior in official help -> 10+ varied searches, relevant full pages, exact UI/helper text, blocked notes, CLAIMS and EXPAND tails | task `thefork_closure_availability` | agent `/root/thefork_closure_availability` [pending]
- **F** "evidence-skeptic" (perspective) - Counter-search claimed labels/pages, identify contradictions, stale wording, snippets-only evidence, and unchecked leads -> Independent 10+ query audit, source-quality verdicts, counterevidence, CLAIMS and EXPAND tails | task `thefork_evidence_skeptic` | agent `/root/thefork_evidence_skeptic` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-d526328f/artifacts`
