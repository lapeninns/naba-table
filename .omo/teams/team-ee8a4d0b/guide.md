# Team quandoo-timing - Member Field Manual
...
- Team: **quandoo-timing** (id `6c460310-caca-40e1-ac99-a4f9aad1b399`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-ee8a4d0b/team.json`
...
**You are a member of team quandoo-timing, not a generic assistant.** You own exactly one
...
- **C** "widget-consumer" (perspective) - Official Quandoo booking widgets and restaurant listing behavior to test latest reservable start versus duration/end-of-shift computation -> 10+ official/domain searches, inspect full pages and live widgets where possible, empirical timing observations, URLs, gaps, CLAIMS and EXPAND | task `quandoo_widget_behavior` | agent `/root/quandoo_widget_behavior` [active]
- **E** "localized-archive" (area) - Official localized Quandoo domains, sitemaps, archived/help pages and PDFs for timing semantics absent in English -> 10+ distinct official searches across languages, fetch pages, translate exact labels faithfully, URLs/access, gaps, CLAIMS and EXPAND | task `quandoo_localized_archive` | agent `/root/quandoo_localized_archive` [active]
- **F** "skeptic-countersearch" (perspective) - Actively seek official Quandoo evidence refuting or qualifying start-cutoff, duration, closing-time and after-close-finish interpretations -> 10+ distinct official counter-queries, full-page sources, contradictions/login barriers, CLAIMS and EXPAND | task `quandoo_countersearch` | agent `/root/quandoo_countersearch` [active]
- **A** "help-centre-settings" (area) - Official Quandoo Help Centre and operator settings semantics for last reservation, duration, closing, table occupancy -> 10+ official searches, full-page evidence, exact labels/helper text, quotes under 20 words, URLs, gaps, CLAIMS and EXPAND | task `quandoo_help_settings` | agent `/root/quandoo_help_settings` [active]
- **D** "table-plan-mobile" (area) - Official Quandoo table management, seating, availability, duration and mobile app documentation -> 10+ official searches, full-page evidence, labels/helper text, URLs, gaps, CLAIMS and EXPAND | task `quandoo_table_mobile` | agent `/root/quandoo_table_mobile` [active]
- **B** "partner-product" (area) - Official Quandoo partner/business product pages, manuals, PDFs, training and resource material about operating hours and reservations -> 10+ official searches, full-page evidence, exact wording, URLs, gaps, CLAIMS and EXPAND | task `quandoo_partner_product` | agent `/root/quandoo_partner_product` [active]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-ee8a4d0b/artifacts`
