# Team thefork-redirect-verifiers - Member Field Manual
...
- Team: **thefork-redirect-verifiers** (id `c88f28eb-74ce-4a17-b3b7-74a00ca4bbdd`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-70dd4ba1/team.json`
...
**You are a member of team thefork-redirect-verifiers, not a generic assistant.** You own exactly one
...
- **A** "manager-schemes" (area) - manager.thefork.com and manager.thefork.co.uk scheme and www/non-www redirect chains -> At least 10 direct official HTTP probes with exact chain, Location, final URL, canonical, access date, claims and EXPAND tail. | task `manager_schemes` | agent `/root/manager_schemes` [pending]
- **B** "help-schemes" (area) - help.thefork.com and official support host scheme and hostname variants -> At least 10 direct official HTTP probes with exact chain, Location, final URL, canonical, access date, claims and EXPAND tail. | task `help_schemes` | agent `/root/help_schemes` [pending]
- **C** "locale-paths" (area) - locale-less and alternate locale paths on manager/help official hosts -> At least 10 direct official HTTP probes across en-GB, en, fr, de, es, it, nl or discovered locales; exact chains and canonicals; claims and EXPAND. | task `locale_paths` | agent `/root/locale_paths` [pending]
- **D** "article-aliases" (area) - known and discoverable official TheFork help article aliases and manager help paths -> Discover aliases from official sitemap/search and probe at least 10 paths, recording exact chains/finals/canonicals and EXPAND. | task `article_aliases` | agent `/root/article_aliases` [pending]
- **E** "normalization" (perspective) - trailing slash, query parameter, fragment-independent and percent/case normalization on official manager/help URLs -> At least 10 paired direct probes with exact redirect preservation/removal behavior, final URL and canonical, claims and EXPAND. | task `normalization_variants` | agent `/root/normalization_variants` [pending]
- **F** "counter-verifier" (perspective) - independent red-team repeat of representative manager/help chains using GET and HEAD, different user agents, loop and off-domain detection -> At least 10 independent probes, discrepancies versus expected behavior, final URL/canonical, claims and EXPAND. | task `counter_verifier` | agent `/root/counter_verifier` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-70dd4ba1/artifacts`
