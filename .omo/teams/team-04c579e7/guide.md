# Team square-availability-errors - Member Field Manual
...
- Team: **square-availability-errors** (id `4bc64e3a-c197-4fb2-bda3-888cc09254ae`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-04c579e7/team.json`
...
**You are a member of team square-availability-errors, not a generic assistant.** You own exactly one
...
- **A** "endpoint-contract" (area) - SearchAvailability endpoint reference, request body, response body, examples, and explicit endpoint errors -> Official Square-only evidence; at least 10 distinct searches; full pages; exact names/descriptions and quotes under 20 words; query log, dead ends, CLAIMS and EXPAND | task `sq_endpoint_contract` | agent `/root/sq_endpoint_contract` [pending]
- **B** "error-schema" (area) - Square standard Error schema, ErrorCategory, ErrorCode, HTTP status and endpoint error response linkage -> Official Square-only schema evidence; at least 10 searches; full pages and linked schemas; exact names/descriptions, query log, dead ends, CLAIMS and EXPAND | task `sq_error_schema` | agent `/root/sq_error_schema` [pending]
- **C** "time-validation" (perspective) - Invalid start time, end time, time range, missing time fields, date-time format, and boundary validation counter-searches -> Official Square-only evidence; at least 10 counter-searches; full pages; distinguish documented from absent; query log, dead ends, CLAIMS and EXPAND | task `sq_time_validation` | agent `/root/sq_time_validation` [pending]
- **D** "resource-validation" (area) - Location, service variation, segment filters, team member, merchant and version validation in SearchAvailability -> Official Square-only evidence; at least 10 searches; full pages; exact code/name/error text; query log, dead ends, CLAIMS and EXPAND | task `sq_resource_validation` | agent `/root/sq_resource_validation` [pending]
- **E** "empty-unavailable" (perspective) - No availability, unavailable slots/resources, empty availabilities arrays, valid empty response, and absence-of-error behavior -> Official Square-only evidence; at least 10 searches and counter-searches; full pages; clearly distinguish HTTP error from valid empty response; query log, dead ends, CLAIMS and EXPAND | task `sq_empty_unavailable` | agent `/root/sq_empty_unavailable` [pending]
- **F** "product-scope" (perspective) - Square Bookings/Appointments applicability, restaurant/reservation counter-search, API access limitations and current product scope -> Official Square-only evidence; at least 10 searches; full pages; appointments versus restaurants conclusion; query log, dead ends, CLAIMS and EXPAND | task `sq_product_scope` | agent `/root/sq_product_scope` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-04c579e7/artifacts`
