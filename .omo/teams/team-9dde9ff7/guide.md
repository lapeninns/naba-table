# Team thefork-official-duration - Member Field Manual
...
- Team: **thefork-official-duration** (id `bbd52776-9faf-4302-ba96-539edc534715`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-9dde9ff7/team.json`
...
**You are a member of team thefork-official-duration, not a generic assistant.** You own exactly one
...
- **A** "help-manuals" (area) - Official TheFork Help Centre/manual articles on reservation duration, table plan, services, booking intervals, closing hours -> 10+ official-only queries; opened pages; quotes under 20 words; URLs/titles/access date; silence/counter-search; CLAIMS and EXPAND | task `official_help_manuals` | agent `/root/official_help_manuals` [archived]
- **B** "academy-training" (area) - Official TheFork Academy, blog, webinar, tutorial, video, training resources on reservation duration and service scheduling -> 10+ official-only queries; opened resources; quotes; URLs/titles/access date; dead ends; CLAIMS and EXPAND | task `official_academy_training` | agent `/root/official_academy_training` [archived]
- **C** "pdf-sitemap" (area) - Official TheFork PDFs, manuals, downloadable guides, sitemaps and indexed resource discovery -> 10+ official-only queries including filetype:pdf and sitemap; open PDFs/pages; quotes; URLs/titles/access date; dead ends; CLAIMS and EXPAND | task `official_pdf_sitemap` | agent `/root/official_pdf_sitemap` [archived]
- **D** "closing-countersearch" (perspective) - Official TheFork counter-search for whether reservation duration may extend past service or restaurant closing time; seek explicit statement or silence -> 10+ official-only queries; opened pages; explicit positive/negative evidence; quote limits; access date; query log and dead ends; CLAIMS and EXPAND | task `official_closing_countersearch` | agent `/root/official_closing_countersearch` [archived]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-9dde9ff7/artifacts`
