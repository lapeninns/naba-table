# Team google-actions-center-eligibility - Member Field Manual
...
- Team: **google-actions-center-eligibility** (id `bf0ebaa1-ddd0-469d-b446-e81ef471a9b4`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-dca05f29/team.json`
...
**You are a member of team google-actions-center-eligibility, not a generic assistant.** You own exactly one
...
- **A** "partner-onboarding" (ownership) - Actions Center partner prerequisites, onboarding, account and integration requirements -> Official Google-only evidence with 10+ queries, full-page reads, quotes, countersearches, CLAIMS and EXPAND | task `partner_onboarding` | agent `/root/partner_onboarding` [pending]
- **B** "merchant-eligibility" (ownership) - Merchant and business eligibility policies for Reserve with Google/Actions Center -> Official Google-only evidence with 10+ queries, full-page reads, quotes, countersearches, CLAIMS and EXPAND | task `merchant_eligibility` | agent `/root/merchant_eligibility` [pending]
- **C** "restaurant-coverage" (area) - Explicit restaurant, dining, food category, country, and geography coverage -> Official Google-only evidence with 10+ queries, full-page reads, quotes, countersearches, CLAIMS and EXPAND | task `restaurant_coverage` | agent `/root/restaurant_coverage` [pending]
- **D** "restriction-countersearch" (perspective) - Unsupported, disqualifying, restricted, prohibited, and non-eligible restaurant cases -> Official Google-only evidence with 10+ queries, full-page reads, quotes, countersearches, CLAIMS and EXPAND | task `restriction_countersearch` | agent `/root/restriction_countersearch` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-dca05f29/artifacts`
