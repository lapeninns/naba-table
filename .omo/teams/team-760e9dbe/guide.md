# Team resdiary-sitemap-research - Member Field Manual
...
- Team: **resdiary-sitemap-research** (id `7d490504-6c85-47cf-b396-cfc7a9d3380e`)
- Transport: **MultiAgentV2 (native agents: flat `spawn_agent` / `send_message` / `followup_task`)**
- Team state: `/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-760e9dbe/team.json`
...
**You are a member of team resdiary-sitemap-research, not a generic assistant.** You own exactly one
...
- **A** "robots-sitemaps" (area) - Official ResDiary robots.txt, sitemap.xml, sitemap indexes, nested sitemap discovery and URL extraction -> Complete official robots/sitemap inventory, parsed relevant URLs, query ledger, statuses, claims and EXPAND tail | task `resdiary_robots_sitemaps` | agent `/root/resdiary_robots_sitemaps` [pending]
- **B** "help-taxonomy" (area) - Official ResDiary help-center home, category, collection, search, pagination, related-link and index taxonomy -> Complete help index/category/search taxonomy with pagination and operator-rich URLs, query ledger, statuses, claims and EXPAND tail | task `resdiary_help_taxonomy` | agent `/root/resdiary_help_taxonomy` [pending]
- **C** "schedule-services" (ownership) - Operator availability schedule, opening hours, services, shifts, sessions and availability configuration documentation -> Official pages for schedule/services terminology and configuration, query ledger, region/version/status, claims and EXPAND tail | task `resdiary_schedule_services` | agent `/root/resdiary_schedule_services` [pending]
- **D** "turn-end-time" (ownership) - Operator turn time, table duration, last seating, end time, close time, booking duration documentation -> Official pages for turn-time and last-seating/end-time concepts, query ledger, status/redirect, claims and EXPAND tail | task `resdiary_turn_end_time` | agent `/root/resdiary_turn_end_time` [pending]
- **E** "pacing-cutoff" (ownership) - Operator pacing, covers per interval, booking interval, booking cutoff, minimum advance notice and lead-time documentation -> Official pages for pacing and cutoff/lead-time configuration, query ledger, status/redirect, claims and EXPAND tail | task `resdiary_pacing_cutoff` | agent `/root/resdiary_pacing_cutoff` [pending]
- **F** "legacy-region-skeptic" (perspective) - Legacy help sections, alternate official domains/subdomains, country/region/version clues, redirects/dead pages, missing-terminology counter-search -> Skeptical completeness audit with official-only legacy/region URL taxonomy, redirect/dead status checks, 10+ counter-searches, claims and EXPAND tail | task `resdiary_legacy_region` | agent `/root/resdiary_legacy_region` [pending]
...
`/Users/amankumarshrestha/LapenInns Project/nabatableLP/.omo/teams/team-760e9dbe/artifacts`
