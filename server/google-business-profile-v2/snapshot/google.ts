 ...
 *
 * Google-side snapshot reader. Wraps the legacy
 * `readGoogleBusinessProfileBusinessInfo` and reduces its output to the V2
 * canonical-section value shapes.
 *
 * Day-of-week mapping mirrors the legacy convention (`SUNDAY = 0` ... `SATURDAY = 6`).
 */

import { readGoogleBusinessProfileBusinessInfo } from '@/server/google-business-profile/business-info';

import { finalizeSnapshot, type NabatableSnapshotResult } from './nabatable';
...
export type GoogleSnapshotResult = NabatableSnapshotResult;

export async function readGoogleSnapshot({
  client,
  restaurantId,
}: ReadGoogleSnapshotInput): Promise<GoogleSnapshotResult> {
...
): SyncV2ProfileSectionValue['storefrontAddress'] {
  const addresses = info.addresses ?? [];
  const primary = addresses.find((a) => a.isPrimary) ?? addresses[0];
  if (!primary) return null;
  return {
...
function extractOperatingHours(
  ...
  const weekly: SyncV2OperatingHoursDay[] = info.coreNormalization.operatingHours.weekly.map(
    (entry) => ({
...
function extractServicePeriods(
  ...
  const periods: SyncV2ServicePeriod[] = info.coreNormalization.servicePeriods.periods
    .map((entry) => ({
      stableKey: [
