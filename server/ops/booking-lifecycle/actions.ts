...
export function prepareCheckInTransition(options: CheckInOptions): TransitionResult {
  ...
  const shouldOverwriteTimestamp = !booking.checked_in_at;
  ...
  const resolvedCheckedInAt =
    'checked_in_at' in updates
      ? (updates.checked_in_at ?? null)
      : (booking.checked_in_at ?? performedIso);
...
export function prepareCheckOutTransition(options: CheckOutOptions): TransitionResult {
  ...
  const shouldOverwrite = !booking.checked_out_at;
  if (shouldOverwrite) {
  ...
  const resolvedCheckedOutAt =
    'checked_out_at' in updates ? (updates.checked_out_at ?? null) : booking.checked_out_at;
...
export function prepareUndoNoShowTransition(options: UndoNoShowOptions): TransitionResult {
  ...
  const nextStatus: BookingStatus =
    typeof targetStatus === 'string' ? (targetStatus as BookingStatus) : 'confirmed';
  ...
  const withinGrace = Number.isFinite(minutesSinceNoShow)
    ? minutesSinceNoShow <= graceMinutes
    : false;
