...
export const BOOKING_STATUSES = ["pending", "pending_allocation", "confirmed", "cancelled", "completed", "no_show", "checked_in", "PRIORITY_WAITLIST"] as const;
...
function createEnumPredicate<T extends readonly string[]>(values: T) {
...
export function ensureBookingType(value: string, fieldName = "booking type"): BookingType {
  if (!isBookingType(value)) {
    throw new Error(`Invalid ${fieldName}: ${value}. Valid values: ${formatValidList(BOOKING_TYPES)}.`);
  }
  return value;
}
...
export function ensureBookingStatus(value: string, fieldName = "booking status"): BookingStatus {
  if (!isBookingStatus(value)) {
    throw new Error(`Invalid ${fieldName}: ${value}. Valid values: ${formatValidList(BOOKING_STATUSES)}.`);
  }
  return value;
}

export function ensureSeatingPreference(value: string, fieldName = "seating preference"): SeatingPreference {
  if (!isSeatingPreference(value)) {
    throw new Error(`Invalid ${fieldName}: ${value}. Valid values: ${formatValidList(SEATING_PREFERENCES)}.`);
  }
  return value;
}
