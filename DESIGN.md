...
- **Focused-mode chrome (ops restaurant settings):** one row, `--ops-chrome-height` (48px) minimum, never fixed or sticky — the page content is the only scroll container, so the chrome cannot cover content, menus or focus. Order: menu trigger (below 768px) → location → status → close. The chrome owns the page's only `h1`; ancestors appear as room allows (parent section as an eyebrow line above the title below 1024px, then an inline trail; the settings root joins it from 1280px). The page body adds a purpose line only when its content does not already open with one. Status that is repeated elsewhere is shown once per viewport: Google status is a chrome pill from 768px and an in-page strip only when there is drift to act on. The restaurant name lives in the sidebar header and moves to the chrome only when the sidebar is collapsed to its rail. Chrome, docked section tabs and page content share one horizontal gutter, `--ops-shell-gutter` (16 → 24 → 32px at 640 and 1024px), so their left edges align. Environment notices sit above the chrome.
