...
- **Pull requests.** After the local check succeeds and all required hosted lanes complete, the operational Worker dispatches the gate even if a hosted lane failed. The trusted gate re-reads the results and publishes an explicit failure. A newer hosted run or attempt permits another dispatch, so a successful rerun can recover the gate. Persisted run-id/attempt fingerprints coalesce duplicate deliveries; incomplete reruns withdraw queued work, stale events are ignored, and asynchronous results cannot erase newer queued attempts or reopen closed/superseded candidates. A new SQLite table is added in place, with existing ready jobs backfilled; no candidate state is deleted.
