...
| `BACKUP_ENCRYPTION_KEY`, `BACKUP_S3_ENDPOINT`, `BACKUP_S3_REGION`, `BACKUP_S3_DRILL_*` | Decryption key and bucket credentials with reads for `backups/` plus writes limited to `evidence/recovery/` for publishing drill evidence (exposed as `BACKUP_S3_ACCESS_KEY_ID` / `BACKUP_S3_SECRET_ACCESS_KEY`). The `Production` environment holds a separate read-only pair, `BACKUP_S3_READ_*`, for `recovery:evidence:check`.                                          |
