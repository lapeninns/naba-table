...
export const RISK_REQUIRED_SCRIPTS = Object.freeze({
  ...
  'customer-pii': {
    always: ['governance:check', 'test'],
    whenPresent: ['lint', 'typecheck', 'build', 'qa:observability-privacy'],
  },
  'auth-session': {
    always: ['governance:check', 'test'],
  ...
  webhooks: {
    always: ['governance:check', 'test'],
    whenPresent: ['lint', 'typecheck', 'build', 'qa:background-workers'],
    durableProof: true,
  },
  migrations: {
    always: ['governance:check', 'test'],
  ...
  'rls-rpc-ledger': {
    always: ['governance:check', 'test'],
    whenPresent: ['lint', 'typecheck', 'build', 'db:check-drift'],
    durableProof: true,
  },
});
...
export const KNOWN_MANUAL_INSPECTION_GATE_PATTERNS = Object.freeze([/^manual:[a-z][a-z0-9-]*$/]);
...
export const RISK_RADIUS_HINTS = Object.freeze([
  { pattern: 'supabase/migrations/**', classes: ['migrations'] },
  { pattern: 'src/app/api/webhook/**', classes: ['webhooks'] },
]);
