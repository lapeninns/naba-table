...
async function runBaseline() {
  ...
  try {
    ...
    const { data: statusCounts } = await supabase.from('bookings').select('status').limit(10000);
  ...
  // Save to JSON file
  const outputDir = path.join(__dirname, '../test-results/performance/db-perf-optimization');
  ...
  if (bookingsOverview?.data[0]) {
    const data = bookingsOverview.data[0] as {
      total_count: number;
