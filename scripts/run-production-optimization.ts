...
if (!connectionString) {
    ...
    console.error("   Usage: DB_URL=\"postgresql://...\" npx tsx scripts/run-production-optimization.ts");
    process.exit(1);
}
...
async function runPhase(client: Client, phaseName: string, commands: string[]): Promise<PhaseResult> {
...
async function main() {
    ...
    try {
        ...
        for (const res of results) {
            ...
            console.log(`  ${status} ${res.phase}: ${res.commands - res.errors.length}/${res.commands} OK (${res.duration}ms)`);
        }
