...
function writePasswordToGitignoredBackups(repoRoot: string, email: string, password: string): string {
...
async function main() {
  ...
  if (created.error) {
    ...
    if (!existing?.id) {
      throw new Error(`User appears to exist but could not be found by listUsers(). Original error: ${msg}`);
    }
    userId = existing.id;
  } else {
    createdNewUser = true;
  ...
  const restaurantsRes = await service.from("restaurants").select("id, name, slug").order("name", { ascending: true });
  ...
  if (signIn.data.session) {
    ...
    if (setSession.error) {
      ...
      if (rlsRestaurants.error) {
        console.warn("[bootstrap-owner] RLS restaurant list check failed.", { message: rlsRestaurants.error.message });
      } else if ((rlsRestaurants.data ?? []).length === 0) {
        console.warn("[bootstrap-owner] RLS restaurant list returned 0 rows; verify RLS policies/memberships.");
      }
    }
  }
