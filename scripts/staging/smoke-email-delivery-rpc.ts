...
type SummaryRow =
  Database['public']['Functions']['ops_email_delivery_attempts_summary']['Returns'][number];
type FeedRow =
  Database['public']['Functions']['ops_email_delivery_attempts_feed']['Returns'][number];
...
async function main() {
  ...
  assertStagingScriptSafety({
    ...
    confirmation: process.env.CONFIRM_STAGING_EMAIL_DELIVERY_SMOKE,
    confirmationName: 'CONFIRM_STAGING_EMAIL_DELIVERY_SMOKE',
  });
  ...
  if (sample) {
    console.log(
      `- Feed sample: messageId=${sample.messageId} recipientEmailPresent=${Boolean(sample.recipientEmail)} currentStatus=${sample.currentStatus}`,
    );
  }
}
...
main().catch((error) => {
  console.error(
    '[smoke-email-delivery-rpc] Failed:',
