...
BEGIN
  ...
  IF r1_cust1 IS NULL THEN
    ...
    VALUES (r1, 'alice@example.com', '+44 7700 900001', 'Alice Johnson', true)
  ...
  IF NOT FOUND THEN
    ...
    VALUES (r1_cust1, true, '{"seating":"window","allergies":["nuts"]}'::jsonb);
  END IF;
  ...
  IF r1_cust2 IS NULL THEN
    ...
    VALUES (r1, 'bob@example.com', '+44 7700 900002', 'Bob Smith')
  ...
  -- Points rows (include restaurant_id to satisfy NOT NULL)
  PERFORM 1 FROM public.loyalty_points WHERE program_id = r1_prog AND customer_id = r1_cust1;
  IF NOT FOUND THEN
    INSERT INTO public.loyalty_points (restaurant_id, program_id, customer_id, balance, lifetime_points, tier)
    VALUES (r1, r1_prog, r1_cust1, 50, 50, 'bronze');
  END IF;

  PERFORM 1 FROM public.loyalty_points WHERE program_id = r1_prog AND customer_id = r1_cust3;
  IF NOT FOUND THEN
    INSERT INTO public.loyalty_points (restaurant_id, program_id, customer_id, balance, lifetime_points, tier)
    VALUES (r1, r1_prog, r1_cust3, 120, 120, 'silver');
  END IF;
  ...
  IF r1_bk1 IS NULL THEN
    ...
    VALUES (
      ...
      'Anniversary', 'web', 10,
      gen_random_uuid(), 'r1-alice-7d-1900', true, '{"occasion":"anniversary"}'::jsonb
    )
  ...
  IF r1_bk2 IS NULL THEN
    ...
    VALUES (
      ...
      'web', gen_random_uuid(), 'r1-bob-7d-1915', '{"notes":"birthday"}'::jsonb
    )
  ...
  IF r1_bk3 IS NULL THEN
    ...
    VALUES (
      ...
      'phone', gen_random_uuid(), 'r1-carol-14d-2000', '{"requested_area":"Terrace"}'::jsonb
    )
  ...
  IF NOT FOUND THEN
    INSERT INTO public.loyalty_point_events (
      restaurant_id, program_id, customer_id, booking_id, points_delta, balance_after, reason, metadata
    )
    VALUES (r1, r1_prog, r1_cust1, r1_bk1, 10, 60, 'attendance', '{"awarded_by":"system"}'::jsonb);
  ...
  IF NOT FOUND THEN
    ...
    VALUES (
      ...
      '{"party_size":2,"requested_time":"19:30"}'::jsonb,
  ...
  IF NOT FOUND THEN
    ...
    VALUES ('evt_test_seed_1', 'customer.created', '{"id":"evt_test_seed_1"}'::jsonb, 'processed');
  END IF;
  ...
  IF NOT FOUND THEN
    ...
    VALUES ('seed.completed', 'seed', 'info', '{"restaurants":2,"tables":10}'::jsonb);
  END IF;
END
$$;
...
-- ====================================================================
-- SELECT r.name, count(t.*) AS tables
