...
export function useTeamInvitations({
  ...
  return useQuery<RestaurantInvite[], HttpError>({
    ...
    queryFn: async () => {
      ...
      const data = await fetchJson<unknown>(`/api/app/team/invitations?${search.toString()}`);
...
> {
  ...
  return useMutation({
    mutationFn: async (variables) => {
      ...
      const data = await fetchJson<unknown>('/api/app/team/invitations', {
        method: 'POST',
        headers: {
...
export function useRevokeTeamInvite(): UseMutationResult<RestaurantInvite, HttpError, RevokeInviteVariables> {
  ...
  return useMutation({
    mutationFn: async ({ restaurantId, inviteId }) => {
      const search = new URLSearchParams({ restaurantId });
      const data = await fetchJson<unknown>(`/api/app/team/invitations/${inviteId}?${search.toString()}`, {
        method: 'DELETE',
      });
      const parsed = z
